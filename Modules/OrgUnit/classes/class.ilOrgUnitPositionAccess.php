...
class ilOrgUnitPositionAccess implements ilOrgUnitPositionAccessHandler, ilOrgUnitPositionAndRBACAccessHandler
{
    ...
    public function isTalkReadonlyByUser($user_id, $ref_id) {
        ...
        $talk = new ilObjEmployeeTalk((int)$ref_id);
        if ((int)$talk->getOwner() === $user_id || (int)$user_id === 6) {
            return false;
        }

        $user_posis = $this->ua->getPositionsOfUserId($user_id);
        $target_posis = $this->ua->getPositionsOfUserId($talk->getOwner());
        $possibleSuperiors = [];

        foreach ($target_posis as $pos) {
            $possibleSuperiors += $this->ua->getPossibleSuperiorPositionsByPosition($pos->getId());
        }

        foreach ($user_posis as $pos) {
            foreach ($possibleSuperiors as $posPos) {
                if ($posPos === (int)$pos->getId()) {
                    return false;
                }
            }
        }
    ...
    public function isTalkReadonlyByCurrentUser($ref_id) {
        ...
        return $this->isTalkReadonlyByUser($current_user_id, $ref_id);
    }
}
